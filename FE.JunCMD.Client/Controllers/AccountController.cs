        ...
        public async Task<IActionResult> CheckAccount([FromBody] CheckAccountRequestDTO checkAccountRequestDTO)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Username))
            {
                ...
                if (responseJsonData != null)
                {
                    ...
                    DateTime createDateTicks = responseJsonData.CreatedDate;
                    if (createDateTicks < DateTime.Now.AddHours(-24))
                    {
                        var smsCheck = new SMSDTO
                        {
                            Account = responseJsonData.Login,
                            Sender = phone
                        };
                    ...
                    ResponseDTO? res = await _phoneNumber.GetListPhoneBySiteIDAsync(2); /// Site
